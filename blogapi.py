...
def checkURL(url, proxy=None):
    try: urllib.urlopen(url, proxies=proxy)
...
class Blog(object):
    ...
    """
    def __init__(self, serverapi, username, password, appkey, proxy=None):
        """
        Args:
        ...
        self.appkey = appkey    
        self.methods            = []
        self.proxy = proxy # set this to use the proxy
        self.proxytransport = ProxiedTransport()
        self.proxytransport.set_proxy(proxy)
...
class MetaWeblog(Blog):
    ...
    def __init__(self, serverapi, username, password, appkey, proxy=None):
        Blog.__init__(self, serverapi, username, password, appkey, proxy)
...
class WordPress(MetaWeblog):
    ...
    def __init__(self, serverapi, username, password, proxy=None):
        MetaWeblog.__init__(self, serverapi, username, password, '0x001', proxy)
